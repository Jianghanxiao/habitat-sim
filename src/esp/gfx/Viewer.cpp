...
Viewer::Viewer(const Arguments& arguments)
  ...
  args.addNamedArgument("URDF")
#else
  args.addArgument("URDF")
#endif
      .setHelp("URDF", "URDF file to load")
  ...
  const std::string& urdfFile = args.value("URDF");

  // Locate the file
  if (freopen(urdfFile.c_str(), "r", stdin) == NULL)
  {
  ...
  //       {
  //           std::cout << link_vec[i]->parent_link->link_name << "; " << link_vec[i]->mesh_name;
  //       }
  //       std::cout << "\n";
  //   }
...
void Viewer::loadURDFMesh(Link *node, scene::SceneNode* parent, Magnum::SceneGraph::DrawableGroup3D* drawables)
{
  std::cout << "!!!!\n";
